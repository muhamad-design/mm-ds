...
version: alpha
name: Geist
description: mm-ds, a Geist-based design system - Dark theme (the Light theme is documented at /design.md). Token values follow Vercel's public Geist reference.
colors:
  primary: "#ededed"
  secondary: "#a0a0a0"
  tertiary: "#006efe"
...
This file documents the Dark theme. The Light theme keeps the same token names with different values and lives at `/design.md`. Colors are sRGB hex; the `*-p3` tokens are wide-gamut `oklch()` equivalents for Display P3 screens.
...
Depth comes from borders and tonal surfaces before shadows, so shadows stay quiet. Dark-theme `box-shadow` values:
...
Medium is 40px; `button-small`/`input-small` are 32px and `button-large`/`input-large` are 48px (large buttons step type up to `button-16`). States move along the scale: fills go `100` to `200` on hover and `300` on active; borders go `400` to `500` to `600`. Disabled uses a `gray-100` fill, `gray-700` text, and a not-allowed cursor. Focus is a two-layer ring - `box-shadow: 0 0 0 2px #000000, 0 0 0 4px #47a8ff` - a 2px gap in the surface color, then a 2px `blue-900` ring.
