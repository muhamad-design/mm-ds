...
version: alpha
name: Geist
description: mm-ds, a Geist-based design system - Light theme (the Dark theme is documented at /design.dark.md). Token values follow Vercel's public Geist reference.
colors:
  primary: "#171717"
  secondary: "#4d4d4d"
  tertiary: "#006bff"
...
This file documents the Light theme. The Dark theme keeps the same token names with different values and lives at `/design.dark.md`. Colors are sRGB hex; the `*-p3` tokens are wide-gamut `oklch()` equivalents for Display P3 screens.
...
Depth comes from borders and tonal surfaces before shadows, so shadows stay quiet. Light-theme `box-shadow` values:
...
Medium is 40px; `button-small`/`input-small` are 32px and `button-large`/`input-large` are 48px (large buttons step type up to `button-16`). States move along the scale: fills go `100` to `200` on hover and `300` on active; borders go `400` to `500` to `600`. Disabled uses a `gray-100` fill, `gray-700` text, and a not-allowed cursor. Focus is a two-layer ring - `box-shadow: 0 0 0 2px #ffffff, 0 0 0 4px #006bff` - a 2px gap in the surface color, then a 2px `blue-700` ring.
